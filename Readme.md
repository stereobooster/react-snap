...
1. Use [react-prerendered-component](https://github.com/theKashey/react-prerendered-component)
   This library could _keep_ the pre-rendered HTML until codesplitted part is ready.
...
const prerenderedLoadable = dynamicImport => {
  ...
  // you can use `.preload()` method for react-loadable or react-imported-component
...
`MyComponent` will use prendered html to prevent flash (it will find needed piece of html using `id` generated by `PrerenderedComponent` and inject it using `dangerouslySetInnerHTML`).

2. The same apporoach will work with `React.lazy`, but `React.lazy` doesn't provide prefetch method (`load` or `preload`), so you need to implement it yourself (fragile)
